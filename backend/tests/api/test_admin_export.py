...
async def test_export_logs_csv_success(test_client, override_admin_auth):
    with patch("app.api.v1.endpoints.admin.log_service.get_all_logs") as mock_get_logs, \
...
async def test_export_with_filters(test_client, override_admin_auth):
     with patch("app.api.v1.endpoints.admin.log_service.get_all_logs") as mock_get_logs, \
        ...
        user_id = uuid4()
        await test_client.get(f"/api/v1/admin/export?format=csv&user_id={user_id}&min_calories=100&max_calories=500")
