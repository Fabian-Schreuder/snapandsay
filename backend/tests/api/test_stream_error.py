...
def override_auth():
    ...
    user_context = UserContext(
        id=user_id, aud="authenticated", role="authenticated", email="test@example.com", app_metadata={}
    )
...
async def test_stream_error_updates_db_status(test_client, db_session, override_auth):
    ...
    with (
        ...
        payload = {"log_id": str(log_id), "image_path": f"{user_id}/test.jpg"}
        ...
        result = await db_session.execute(select(DietaryLog).where(DietaryLog.id == log_id))
