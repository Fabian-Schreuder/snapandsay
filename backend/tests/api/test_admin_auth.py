...
async def test_admin_by_role():
    user = UserContext(id=uuid4(), aud="authenticated", role="authenticated", app_metadata={"role": "admin"})
...
async def test_admin_by_email(mock_admin_settings):
    user = UserContext(id=uuid4(), aud="authenticated", role="authenticated", email="admin@example.com")
    result = await get_current_admin(user)
    assert result == user
...
async def test_not_admin(mock_admin_settings):
    user = UserContext(id=uuid4(), aud="authenticated", role="authenticated", email="user@example.com")
