...
async def get_logs(
    ...
    for log in logs:
        ...
        try:
            ...
            signed_url = supabase.storage.from_("raw_uploads").create_signed_url(log.image_path, 3600)
    ...
    return DietaryLogListResponse(data=log_responses, meta=LogListMeta(total=len(log_responses)))
...
async def update_log(
    ...
    try:
        ...
        supabase = get_supabase_client()
        signed_url = supabase.storage.from_("raw_uploads").create_signed_url(updated.image_path, 3600)
        response_model.image_url = signed_url["signedURL"]
    except Exception:
        pass
