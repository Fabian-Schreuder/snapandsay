...
class OracleRunner:
    ...
    async def _process_loop(self, log_id: str, dish: NutritionDish, headers: Dict, image_url: str) -> Dict[str, Any]:
        """
        ...
        try:
            async with self.client.stream("POST", "/api/v1/analysis/stream", json=stream_payload, headers=headers) as response:
                if response.status_code != 200:
                    return {"dish_id": dish.dish_id, "success": False, "error": f"Stream failed: {response.status_code}", "log_id": log_id}
