...
#### DF-029: Two-Phase Sweep Design as Methodological Contribution
- **Source:** HR+TR pass on threshold calibration additions (2026-03-11)
- **Decision:** The split between a full-distribution phase (N=500 for TNR) and a stratified Complex-only phase (N=80 for TPR) is not merely an implementation choice — it is a methodological response to the statistical power problem revealed by the stratification audit. The design is directly caused by the left-skewed score distribution finding (mean=0.194, median=0.125, max=0.686).
- **Discussion relevance:** The evaluation methodology itself demonstrates that the stratification engine's miscalibration (3.7% → 27% Complex after audit) was not just a bug-fix but a prerequisite for obtaining statistically valid TPR estimates. This chain (score distribution audit → stratification fix → two-phase sweep design → valid Pareto analysis) is a complete evaluation integrity narrative worth highlighting in §6.4.7 (Threats to Validity) as evidence that the threat was identified and mitigated.
- **Status (2026-03-11):** RESOLVED. Two sentences added to §6.4.7 internal validity paragraph closing the evaluation integrity narrative: audit → correction → two-phase design adaptation → valid Pareto analysis. Framed as evidence that circularity threat was identified *in situ* and systematically mitigated. All 29 DF items now resolved.
