        ...
        <div data-testid="webcam-mock">
          <button
            onClick={() =>
              ...
              props.onUserMediaError({ name: "OverconstrainedError" })
            }
          >
            Trigger Overconstrained Error
          </button>
          <button
            onClick={() =>
              props.onUserMediaError &&
              props.onUserMediaError({ name: "NotAllowedError" })
            }
          >
          ...
          <button
            onClick={() =>
              props.onUserMediaError && props.onUserMediaError("Unknown Error")
            }
          >
...
describe("CameraCapture", () => {
  ...
  it("renders the shutter button", () => {
    render(<CameraCapture onCapture={jest.fn()} />);
    expect(
      screen.getByRole("button", { name: /shutter/i }),
    ).toBeInTheDocument();
  });
  ...
  it("shows specific error UI when resolution is not supported", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Camera resolution not supported/i),
      ).toBeInTheDocument();
    });
  });
  ...
  it("retries by resetting state instead of reloading page", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId("webcam-mock")).toBeInTheDocument();
      expect(
        screen.queryByText(/Camera Access Needed/i),
