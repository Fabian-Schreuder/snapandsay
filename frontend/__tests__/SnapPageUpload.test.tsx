...
jest.mock("@/components/features/camera/CameraCapture", () => {
  return function MockCameraCapture({ onCapture }: any) {
    return (
      <button onClick={() => onCapture("data:image/jpeg;base64,fake")}>
        Capture Photo
      </button>
    );
  };
...
jest.mock("@/components/features/voice/VoiceCaptureButton", () => ({
  VoiceCaptureButton: function MockVoiceCaptureButton({
    onRecordingComplete,
  }: any) {
    return (
      <button
...
describe("SnapPage Upload Flow", () => {
  ...
  beforeEach(() => {
    ...
    (uploadService.generateUploadPath as jest.Mock).mockImplementation(
      (uid, type) => `${uid}/mock_${type}`,
    );
    (analysisApi.upload as jest.Mock).mockResolvedValue({
      log_id: "123",
      status: "processing",
    });
  ...
  it("completes the full flow: capture -> confirm -> record -> upload", async () => {
    ...
    expect(uploadService.uploadFile).toHaveBeenCalledWith(
      "raw_uploads",
      "test-user-id/mock_image",
      expect.any(Blob),
    );
    expect(uploadService.uploadFile).toHaveBeenCalledWith(
      "raw_uploads",
      "test-user-id/mock_audio",
      expect.any(Blob),
    );
    ...
    await waitFor(
      ...
      { timeout: 3000 },
    ); // Increase timeout for the 1500ms delay in component
  });
